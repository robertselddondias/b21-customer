...
class FireStoreUtils {
          ...
          if (userDocument.data() != null && userDocument.exists) {
            try {
              ...
              WalletTransactionModel transactionModel = WalletTransactionModel(
                  ...
                  paymentType: orderModel.creditCard?.cardHolderName,
  ...
  static setCreditCard(CreditCardUserModel creditCardModel) async {
    bool isAdded = false;
    await fireStore.collection(CollectionName.creditCard).doc(creditCardModel.id).set(creditCardModel.toJson()).then((value) {
      isAdded = true;
    }).catchError((error) {
      log("Failed to update CreditCardModel: $error");
      isAdded = false;
    });
    return isAdded;
  }

  static Future<List<CreditCardUserModel>?> getAllCreditCard(String userId) async {
    List<CreditCardUserModel> creditCardList = [];

    await fireStore.collection(CollectionName.creditCard).where('userId', isEqualTo: userId).get().then((value) {
      for (var element in value.docs) {
        CreditCardUserModel creditCardModel = CreditCardUserModel.fromJson(element.data());
  ...
  static Future<void> deleteCreditCard(String cardId) async {
    await fireStore.collection(CollectionName.creditCard).doc(cardId).delete();
  }
